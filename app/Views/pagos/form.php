...
<?php if (!$esEdicion): ?>
    <script>
        const selectLectura = document.getElementById('lectura_id');
        const inputMonto = document.getElementById('monto');

        function actualizarMonto() {
            const option = selectLectura.options[selectLectura.selectedIndex];

            if (option && option.dataset.monto) {
                inputMonto.value = 'Q ' + Number(option.dataset.monto).toFixed(2);
            } else {
                inputMonto.value = 'Seleccione una lectura';
            }
        }

        selectLectura.addEventListener('change', actualizarMonto);

        // Cargar automáticamente el monto de la lectura
        // que ya viene seleccionada desde Lecturas.
        actualizarMonto();
    </script>
<?php endif; ?>
<?= $this->endSection() ?>